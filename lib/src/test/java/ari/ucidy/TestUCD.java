...
class TestUCD {
	...
	void testListErrors(){
		...
		/* CASE: Deprecated words */

		ucd = new UCD(new UCDWord[]{ new UCDWord("pos.eop.nutation", new UCD(new UCDWord[]{ new UCDWord(UCDSyntax.PRIMARY, "pos.nutation", null, true) }), null, null) });
		ucd.listErrors();
		assertEquals(1, ucd.errors.length);
		assertEquals("1 DEPRECATED UCD word: \"pos.eop.nutation (-> pos.nutation)\"!", ucd.errors[0]);

		ucd = new UCD(new UCDWord[]{ new UCDWord(UCDSyntax.PRIMARY, "phot.flux", null, true), new UCDWord("em.IR.K.Brgamma", new UCD(new UCDWord[]{ new UCDWord(UCDSyntax.SECONDARY, "em.line.Brgamma", null, true) }), null, null) });
		ucd.listErrors();
		assertEquals(1, ucd.errors.length);
		assertEquals("1 DEPRECATED UCD word: \"em.IR.K.Brgamma (-> em.line.Brgamma)\"!", ucd.errors[0]);
	...
	void testGetSuggestion(){
		...
		assertNotNull(suggestion);
		assertTrue(suggestion.isFullyValid());
		assertEquals("pos.nutation", suggestion.toString());
		...
		assertNotNull(suggestion);
		assertTrue(suggestion.isFullyValid());
		assertEquals("phot.flux;em.line.Brgamma", suggestion.toString());
