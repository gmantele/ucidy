...
class TestUCDParser {
	...
	@Test
	void testParseWordListReaderBoolean(){
		try(final InputStream inputWordsStream = UCDWordList.class.getResourceAsStream(UCDParser.FILE_UCD_WORDS)){
			assertNotNull(inputWordsStream);
			try(final Reader reader = new InputStreamReader(inputWordsStream)){
				UCDWordList words = UCDParser.parseWordList(reader, true);
				assertEquals(568, words.size());
				for (UCDWord w : words) {
					if (!w.recommended)
	...
	@Test
	void testParseDeprecatedWordListReaderUCDWordList(){
		try(final InputStream deprecatedWordsStream = UCDWordList.class.getResourceAsStream(UCDParser.FILE_UCD_DEPRECATED)){
			assertNotNull(deprecatedWordsStream);
			...
			try(final Reader reader = new InputStreamReader(deprecatedWordsStream)) {
				...
				for (UCDWord w : lstDeprecatedWords) {
					...
					assertNotNull(w.suggestedReplacement);
	...
	void testParsePSVLine() {
		...
		try{
            ...
            assertInstanceOf(ParseException.class, ex);
	...
	@Test
	void testParseDeprecatedFileLine(){
